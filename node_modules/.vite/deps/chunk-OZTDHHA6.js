...
var NetworkController = {
  ...
  checkIfSupportedNetwork() {
    state8.isUnsupportedChain = !state8.requestedCaipNetworks?.some((network) => network.id === state8.caipNetwork?.id);
    if (state8.isUnsupportedChain) {
      this.showUnsupportedChainUI();
    }
  },
  checkIfSmartAccountEnabled() {
    const networkId = NetworkUtil.caipNetworkIdToNumber(state8.caipNetwork?.id);
    if (!networkId) {
      return false;
    }
    return Boolean(state8.smartAccountEnabledNetworks?.includes(networkId));
  },
  resetNetwork() {
...
var SwapApiUtil = {
  ...
  mapBalancesToSwapTokens(balances) {
    return balances?.map((token) => ({
      ...token,
      address: token?.address ? token.address : `${token.chainId}:${ConstantsUtil2.NATIVE_TOKEN_ADDRESS}`,
...
var SwapController = {
  ...
  getParams() {
    ...
    const networkAddress = `${NetworkController.state.caipNetwork?.id}:${ConstantsUtil2.NATIVE_TOKEN_ADDRESS}`;
    if (!address) {
      throw new Error("No address found to swap the tokens from.");
    }
    const caipAddress = AccountController.state.caipAddress;
    const invalidToToken = !state10.toToken?.address || !state10.toToken?.decimals;
    const invalidSourceToken = !state10.sourceToken?.address || !state10.sourceToken?.decimals || !NumberUtil.bigNumber(state10.sourceTokenAmount).isGreaterThan(0);
    const invalidSourceTokenAmount = !state10.sourceTokenAmount;
    return {
      networkAddress,
      fromAddress: address,
      fromCaipAddress: AccountController.state.caipAddress,
      sourceTokenAddress: state10.sourceToken?.address,
      toTokenAddress: state10.toToken?.address,
      toTokenAmount: state10.toTokenAmount,
      toTokenDecimals: state10.toToken?.decimals,
      sourceTokenAmount: state10.sourceTokenAmount,
      sourceTokenDecimals: state10.sourceToken?.decimals,
      invalidToToken,
      invalidSourceToken,
  ...
  resetValues() {
    ...
    const networkToken = state10.tokens?.find((token) => token.address === networkAddress);
  ...
  async getNetworkTokenPrice() {
    ...
    state10.networkTokenSymbol = token?.symbol || "";
  ...
  async sendTransactionForApproval(data) {
    ...
    try {
      ...
      const error = err;
      state10.transactionError = error?.shortMessage;
      state10.transactionLoading = false;
    }
  },
  async sendTransactionForSwap(data) {
    ...
    try {
      const successMessage = `Swapped ${state10.sourceToken?.symbol} to ${NumberUtil.formatNumberToLocalString(toTokenAmount, 3)} ${state10.toToken?.symbol}!`;
      const forceUpdateAddresses = [state10.sourceToken?.address, state10.toToken?.address].join(",");
...
var ConnectorController = {
  ...
  getConnector(id, rdns) {
    return state14.connectors.find((c) => c.explorerId === id || c.info?.rdns === rdns);
  }
};
  ...
  async getQuote() {
    ...
    try {
      const quote = await BlockchainApiController.getOnrampQuote({
        ...
        amount: state17.paymentAmount?.toString() || "0",
        network: state17.purchaseCurrency?.symbol
      });
...
lit-html/development/directives/ref.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-OZTDHHA6.js.map
