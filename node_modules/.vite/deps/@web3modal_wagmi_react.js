import {
  ens_normalize
} from "./chunk-NTHJK6OW.js";
import {
  mainnet
} from "./chunk-7HS75YHE.js";
import {
  ConstantsUtil,
  HelpersUtil,
  PresetsUtil,
  ...
  y
} from "./chunk-W2T7D5AI.js";
import {
  W3mFrameConstants,
  W3mFrameHelpers,
  W3mFrameRpcConstants
} from "./chunk-2TAEEBCG.js";
import "./chunk-37OJEXSA.js";
import {
  ConstantsUtil2,
  NetworkUtil
...
import "./chunk-4MJOJ2QF.js";
import "./chunk-7KOOD3VM.js";
...
function twistedEdwards(curveDef) {
  ...
  const toAffineMemo = memoized((p, iz) => {
    const { ex: x, ey: y2, ez: z2 } = p;
    const is0 = p.is0();
    if (iz == null)
      iz = is0 ? _8n : Fp4.inv(z2);
    const ax = modP(x * iz);
    const ay = modP(y2 * iz);
    const zz = modP(z2 * iz);
    if (is0)
...
function map_to_curve_elligator2_curve25519(u) {
  ...
  return { xMn: xn, xMd: xd, yMn: y3, yMd: _1n6 };
}
...
var RistPoint = class _RistPoint {
  ...
  static fromHex(hex) {
    ...
    const y2 = mod2(u1 * Dy);
    const t = mod2(x * y2);
    if (!isValid || isNegativeLE(t, P) || y2 === _0n6)
      throw new Error(emsg);
    return new _RistPoint(new ed25519.ExtendedPoint(x, y2, _1n6, t));
  }
  static msm(points, scalars) {
   ...
   */
  toRawBytes() {
    let { ex: x, ey: y2, ez: z2, et: t } = this.ep;
    const P = ed25519.CURVE.Fp.ORDER;
    const mod2 = ed25519.CURVE.Fp.create;
...
var Web3Modal = class extends Web3ModalScaffold {
    ...
    const connectionControllerClient = {
      connectWalletConnect: async (onUri) => {
        ...
        if (siweConfig?.options?.enabled && typeof provider?.authenticate === "function") {
          const { SIWEController, getDidChainId, getDidAddress } = await import("./exports-UXD42WQP.js");
  ...
  syncConnectors(connectors) {
    ...
    filteredConnectors.forEach(({ id, name, type, icon }) => {
      const isCoinbaseRepeated = coinbaseConnector && id === ConstantsUtil.CONNECTOR_RDNS_MAP[ConstantsUtil.COINBASE_CONNECTOR_ID];
  ...
  async syncAuthConnector(connectors) {
    ...
    if (authConnector) {
      ...
      this.addConnector({
        ...
        showWallets: authConnector?.showWallets === void 0 ? true : authConnector.showWallets
      });
    }
  }
